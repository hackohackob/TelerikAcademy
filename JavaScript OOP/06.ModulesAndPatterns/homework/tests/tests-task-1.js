...
describe('Test for Modules and Patterns in JavaScript', function() {
	...
	describe('Pushing Exam Results', function() {
        ...
        it('expect pushExamResults to not throw if given valid scores', function() {
            var jsoop = Object.create(Course)
                .init(getValidTitle(), [getValidTitle()]);
            var id = jsoop.addStudent(getValidName() + ' ' + getValidName());
            id = jsoop.addStudent(getValidName() + ' ' + getValidName());
            ...
            function test(){
                console.log(jsoop.data.students);
                jsoop.pushExamResults([{StudentID:1,score:3},{StudentID:2,score:2},{StudentID:3,score:4},{StudentID:4,score:5}])
                console.log(jsoop.data.students);
            }
            expect(test).to.not.throw();
        });
